...
function sendSensorValuesToAlertsQueue ({readingsMapped, customer_device_id, alert_configs}: any) {
  Object.keys(readingsMapped[customer_device_id]).map(async () => {
    ...
    await global.producer.send({
      topic: 'alerts-topic',
      messages: [
          { key: 'data', value: JSON.stringify({ customer_device_id: customer_device_id, readings: sensorReadings, alert_config: alert_configs.map((x: any) => x.json_rule) }), partition: 0 }
      ]
    })
  })
}